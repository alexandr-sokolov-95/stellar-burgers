...
describe('проверяем доступность приложения', function () {
  ...
  it('должны загружаться ингридиенты', () => {
    cy.get('[data-testid="ingredient"]').should('have.length', 4);
  });

  it('должны добавляться ингридиенты в конструктор', () => {
    cy.get('[data-testid="constructor-element"]').as('constructor');

    cy.get('[data-testid="ingredient"]').each((el, index) => {
      if (index !== 2) {
        cy.wrap(el).within(() => {
  ...
  it('должен создаться и отправиться заказ', () => {
    ...
    cy.wait('@orderBurger').then(() => {
      ...
      cy.get('@constructor').within(() => {
        cy.get('[data-testid="constructor-filled-element"]').should(
          'not.exist'
        );
